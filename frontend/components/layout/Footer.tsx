...
export async function Footer() {
  ...
  const footerMenu = menus.find(
    (m) =>
      ...
      m.name.toLowerCase().includes("pied")
  );

  const o = options?.acfOptions;
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-10 mb-12">
          ...
          <div className="lg:col-span-2">
            {o?.logo?.node && (
              <Link href="/" className="inline-block mb-6" aria-label="Accueil">
                <Image
                  src={o.logo.node.sourceUrl}
                  alt={o.logo.node.altText || "Logo"}
                  width={o.logo.node.width ?? 150}
                  height={o.logo.node.height ?? 40}
                  className="h-10 w-auto brightness-0 invert"
                />
              </Link>
            )}
            <ul className="space-y-2 text-sm">
              {o?.address && <li>{o.address}</li>}
              {o?.phone && (
                <li>
                  <a
        ...
        <div className="border-t border-gray-700/50 pt-8 flex flex-col sm:flex-row justify-between items-center gap-4 text-sm">
          <p>
            {o?.footerText || `© ${year} — Tous droits réservés`}
          </p>
          {o?.footerLinks && o.footerLinks.length > 0 && (
            <ul className="flex flex-wrap gap-6">
              {o.footerLinks.map((link, i) => (
                <li key={i}>
